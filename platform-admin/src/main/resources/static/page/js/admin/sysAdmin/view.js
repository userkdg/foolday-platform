...
layui.use(['table', 'layer', "jquery"], function () {
    ...
    tableIns = table.render({
        elem: '#dataView',
        url: '/admin/sysAdmin/findAllAdmin',
        title: '会员列表',
        method: 'get',
        toolbar: '#toolbar',
        page: true,
        ...
        cols: [
            [
                ...
                {field: 'username', title: '管理员员名', align: 'center'},
                {field: 'phone', title: '管理员账号', align: 'center'},
                {field: 'creatTime', title: '注册时间', align: 'center',
                    templet: '<div>{{dateFormat(d.creatTime, "yyyy-MM-dd hh:mm")}}</div>'},
                {field: 'password', title: '管理的酒店名', align: 'center'},
                {field: 'sysRoleId', title: '角色', align: 'center'},
                {
                    field: 'state', title: '状态', align: 'center', color:'&#xe654;',
                    templet: '<div>{{convertStatus(d.state)}}</div>'
                },
        ...
        switch (obj.event) {
            case 'edit':
                layer.open({
                    type: 2,
                    title: "修改",
                    content: "/admin/sysAdmin/edit?id=" + data.id,
                    resize: false,
                    area: ['600px', '600px']
                });
                break;
            ...
            case 'enable':
                layer.confirm('确认激活吗？', function (confirmIndex) {
                    layer.close(confirmIndex);
                    $.ajax({
                        url: "/admin/sysAdmin/activateAdmin",
                        data: "id=" + data.id,
                        type: "get",
                        dataType: "json",
                        success: function(result) {
